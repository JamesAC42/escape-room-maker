...
class GridBind extends GridBase {
  ...
  coordsExist (destCoords) {
    ...
    Object.keys(this.props.graph.coordinates)
      .forEach(c => {
        const iCoords = this.props.graph.coordinates[c]
  ...
  handleHover (e) {
    ...
    if (room !== null) {
      const { x, y } = this.props.graph.graph[room].coordinates
      const { topLeftX, topLeftY } = this.topLeftFromBase(x, y)
      const minDir = this.minWallDir(cX, cY, topLeftX, topLeftY)
      const closestRoom = minDir === null ? null : room
      let shouldRender = true
      if (minDir !== null) {
        const destCoords = this.moveCoords(x, y, minDir)
        shouldRender =
  ...
  minWallDir (cX, cY, topLeftX, topLeftY) {
    ...
    const rightCenter = { 1: { x: topLeftX + this.state.cellSize, y: yCenter } }
    const bottomCenter = { 2: { x: xCenter, y: topLeftY + this.state.cellSize } }
  ...
  addNewRoom () {
    const room = this.state.activeAddButton.room
    const graph = { ...this.props.graph }
    const coords = graph.graph[room].coordinates

    const direction = this.state.activeAddButton.direction
    const newCoords = this.moveCoords(coords.x, coords.y, direction)
  ...
  drawAddButton () {
    if (this.state.activeAddButton.room !== null) {
      const room = this.state.activeAddButton.room
      const coords = this.props.graph.graph[room].coordinates
      const { topLeftX, topLeftY } = this.topLeftFromBase(coords.x, coords.y)
      const direction = this.state.activeAddButton.direction
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (prevState.activeAddButton.room !== this.state.activeAddButton.room ||
